...
import { rsu_agreement } from './binding/rsu_agreement';
...
describe('[SMART-CONTRACT: AGREEMENTS] RSU Agreement deployment', () => {
  it('Deploy test_binding', async () => {
    await rsu_agreement.deploy(
      share.get_address(),
      employee.get_address(),
      company.get_address(),
      expiration_date,
    ...
    await share.transfer(company.get_address(), rsu_agreement.get_address(), total_shares, { as: company });
  });
});
...
describe('[SMART-CONTRACT: AGREEMENTS] Call execute', () => {
  it("Call 'execute()'", async () => {
    ...
    await rsu_agreement.execute(amount, { as: employee, amount: new Tez(225) });
  });
});

describe('[SMART-CONTRACT: AGREEMENTS] Call terminate', () => {
  it("Call 'terminate()'", async () => {
    await rsu_agreement.terminate({ as: company });
  });
});
...
describe('[SMART-CONTRACT: AGREEMENTS] Call retrieveExpiredShares()', () => {
  it("Call 'retrieveExpiredShares()'", async () => {
    ...
    await rsu_agreement.retrieveExpiredShares({ as: company });
  });
});

describe('[SMART-CONTRACT: AGREEMENTS] Call view_getBalances()', () => {
  it("Call 'view_getBalances()'", async () => {
    await rsu_agreement.view_getBalances({ as: company });
  });
});
